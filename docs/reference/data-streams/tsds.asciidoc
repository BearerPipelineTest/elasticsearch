...
A time series data stream (TSDS) is a <<data-streams,data stream>> option that
models your timestamped metrics data as one or more time series.

// TODO: Replace XX% with actual percentage
Using a TSDS lets you store metrics data more efficiently. In our benchmarks,
Kubernetes metrics data stored in a TSDS uses XX% less disk space than a regular
data stream.
...
The following concepts explain what a time series is and its components.

[discrete]
[[time-series]]
==== Time series

A time series is a sequence of timestamped observations that contain repeated
measurements for a specific entity. Together, these observations let you track
changes to the entity over time. For example, a time series can track:

* CPU and disk usage for a computer
* The price of a stock
* Temperature and humidity readings for a weather sensor

If you plot a time series as a graph, one axis will always be time. Each
observation is a data point. For example, the following graph plots a time
series of temperature readings for a weather sensor.

image::images/data-streams/time-series-chart.svg[align="center"]

In a TSDS, each {es} document represents an observation, or data point, for a
specific time series. While a TSDS typically contains multiple time series, a
...
document's time series. These dimensions don't change across documents in
the same time series.

In {es}, you mark a field as a dimension using the `time_series_dimension`
...
`time_series_metric` mapping parameter. The parameter accepts the following
metric types as values:
...
A TSDS works like a regular <<data-streams,data stream>> with the following key
differences. Keep these differences in mind when working with a TSDS.
...
{es} checks if the write index has met the rollover criteria in the index
lifecycle policy. If not, {es} recalculates the write index's
`index.time_series.end_time` value as:
...
Each document in a TSDS must contain a dimension field that matches the
`index.routing_path` setting.
...
A TSDS doesn't support `index.sort.*` index settings. A TSDS always
